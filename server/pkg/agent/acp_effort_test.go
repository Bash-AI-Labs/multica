...
func TestACPCatalogProviderGate(t *testing.T) {
	...
	// Copilot discovers its model/effort catalog over ACP, then executes through
	// the one-shot CLI. Unlike the ACP providers above, its setting is delivered
	// by buildCopilotArgs as `--effort`, so the picker remains effective without
	// session/set_config_option.
	if !ThinkingControlSupported("copilot") {
		t.Error("copilot should advertise reasoning control through its --effort flag")
	}
}
