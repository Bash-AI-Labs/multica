...
func TestRequireKnownProfile(t *testing.T) {
	...
	t.Run("local mode permits the conventional local profile before config exists", func(t *testing.T) {
		mkProfiles(t)
		t.Setenv("MULTICA_LOCAL_MODE", "true")
		if err := requireKnownProfile("local"); err != nil {
			t.Fatalf("requireKnownProfile(\"local\") = %v, want nil in local mode", err)
		}
	})
