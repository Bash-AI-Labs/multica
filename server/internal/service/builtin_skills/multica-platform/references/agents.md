...
runtime's model catalog). Copilot's fixed provider vocabulary is
`none|low|medium|high|xhigh|max`; the daemon sends the selected value through
its one-shot CLI `--effort` flag. The CLI forwards the token, the server applies
the provider's fixed-enum or safe-token gate, and the daemon performs the exact
