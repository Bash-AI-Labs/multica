...
func (h *Handler) triggerAgentForPRComment(ctx context.Context, installationID int64, repo ghRepo, prNumber int32, branchName string, commentBody string, commentAuthor string) {
	...
	if installationID != 0 {
		...
		// One installation can be bound to several workspaces; route to the
		// workspace that owns this repo rather than the installation's fallback.
		inst := insts[0]
		wsID = h.resolveWorkspaceForRepo(ctx, inst.WorkspaceID, inst.AccountLogin, repo.Owner.Login, repo.Name)
	} else {
