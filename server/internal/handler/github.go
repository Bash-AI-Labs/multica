...
func (h *Handler) triggerAgentForPRComment(ctx context.Context, installationID int64, repo ghRepo, prNumber int32, branchName string, commentBody string, commentAuthor string) {
	// GitHub App installations may be bound to several workspaces. Match the
	// pull-request webhook fan-out so every workspace with a mirrored PR can
	// notify its independently linked issues.
	...
	if installationID != 0 {
		...
		workspaceIDs = make([]pgtype.UUID, 0, len(insts))
		for _, inst := range insts {
			workspaceIDs = append(workspaceIDs, inst.WorkspaceID)
		}
	} else {
		...
		workspaceIDs = make([]pgtype.UUID, 0, len(prs))
		for _, pr := range prs {
			workspaceIDs = append(workspaceIDs, pr.WorkspaceID)
		}
	}
	...
	for _, workspaceID := range workspaceIDs {
		...
		if err != nil {
			slog.Info("github: pr not found for comment trigger", "repo", repo.FullName, "pr", prNumber, "workspace_id", uuidToString(workspaceID), "err", err)
			continue
		}
