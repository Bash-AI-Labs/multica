...
describe("AppConfigSchema cdn_signed drift", () => {
  ...
  it("preserves a true local-mode decision", () => {
    expect(AppConfigSchema.parse({ local_mode_enabled: true }).local_mode_enabled).toBe(true);
  });

  it("defaults omitted or malformed local-mode decisions to false", () => {
    expect(AppConfigSchema.parse({}).local_mode_enabled).toBeUndefined();
    expect(EMPTY_APP_CONFIG.local_mode_enabled).toBe(false);
    expect(
      AppConfigSchema.parse({ local_mode_enabled: "yes" }).local_mode_enabled,
    ).toBe(false);
  });
